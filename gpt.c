...
float* out_t;
float* inp_t; float* weight_t; float* bias_t;
int B_t; int T_t; int C_t; int OC_t;

void matul_forward_work(){
    float* out = out_t;
    float* inp = inp_t; 
    ...
    int b,t;
    mutex_lock(&mutex);
    if(count > (long long)B * (long long)C ){
        mutex_unlock(&mutex);
        return;
    }
